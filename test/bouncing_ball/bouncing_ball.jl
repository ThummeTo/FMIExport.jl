...
if isfile(lockfile) || isfile(logfile)
    ...
    if !isfile(logfile)
        ...
        @test isapprox(ss[100], 0.3456658910552819; atol = 1e-3) # at 0.5s, shortly after first bounce (0.43s)
        @test isapprox(vs[100], 3.0787568528992235; atol = 1e-3)

        @test isapprox(ss[200], 0.6587682981502954; atol = 1e-3)
        @test isapprox(vs[200], -1.8262431471007778; atol = 1e-3)
